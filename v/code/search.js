...
async function show_random_pictures() {
    ...
    const searchsql = await server.exec('path', ['/imagery/v/code/search.sql', true], 'get_file_contents', []);
    //Execute the SQL against the imagery database
    const pictures = await server.exec('database', ['mutall_imagery', false], 'get_sql_data', [searchsql]);
    //
    //For each image row, show the picture in the image area
    for (const picture of pictures)
        show_picture(picture);
}
function imagePop(event) {
...
function show_picture(pic) {
    ...
    img.loading = 'lazy';
    //
    // Set other attributes such as width, height, and src
    img.width = 320;
    img.height = 240;
    //
    //
...
export async function search_pictures(input) {
    ...
    const pictures = await server.exec('database', ['mutall_imagery', false], 'get_sql_data', [SqlSearch]);
    // 
    ...
    const results1 = await server.exec('database', ['mutall_imagery', false], 'get_sql_data', [sqlresults]);
    // 
