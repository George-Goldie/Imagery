...
//
//create class imagery
export class imagery {
  
  constructor(){}
//
// ???
  public async show_random_pictures() {
    //
    //Get the query  for random pictures from sql source file
    const searchsql: string = await server.exec(
      "path",
    ...
    for (const picture of pictures) this.show_picture(picture);
  }
  //
  //???
  private imagePop(event: MouseEvent) {
  ...
  private show_picture(pic: picture) {
    ...
    wordParagraph.textContent = `word: ${pic.word}`;
    //
    //10 Append the image and details to the picture element
    detailsDiv.appendChild(sourceParagraph);
    detailsDiv.appendChild(wordParagraph);
    //
    //11 Get the text area element
    const images: HTMLDivElement = document.getElementById(
      "images"
    ) as HTMLDivElement;
  ...
  public async search_pictures(input: HTMLInputElement) {
    ...
    const SqlSearch = `SELECT i.url, source_name, keyword_name FROM image WHERE concat(i.url, source_name, keyword_name) LIKE '%${searchCriteria}%' limit 100`;
    //
    ...
    const sqlresults = `SELECT COUNT(*) as count FROM (SELECT i.url, source_name, keyword_name FROM image WHERE concat(i.url, source_name, keyword_name) LIKE '%${searchCriteria}%') AS subquery;`;
    //
    // Get the paragraph element by id
    const results: HTMLParagraphElement = document.getElementById(
      "results"
    ...
    results1.forEach((row) => {
      results.innerText = row.count + " results found";
    });
  }
}
